...
const blankEvent: Partial<EventSummary> = { name: '', venue: '', description: '', startAt: '', endAt: '', status: 'DRAFT' }
...
export function AdminEventFormPage() {
  ...
  const save = async (event: FormEvent) => {
    ...
    if (new Date(value.startAt) > new Date(value.endAt)) { setError('행사 시작일은 종료일보다 늦을 수 없습니다.'); return }
    try { const payload = { ...value, startAt: new Date(value.startAt).toISOString(), endAt: new Date(value.endAt).toISOString() }; if (eventId && eventId !== 'new') await adminApi.updateEvent(Number(eventId), payload); else await adminApi.createEvent(payload); void navigate('/admin/events') } catch (caught) { setError(caught instanceof Error ? caught.message : '행사를 저장하지 못했습니다.') }
  }
...
function toLocal(value?: string) { return value ? value.slice(0, 16) : '' }
