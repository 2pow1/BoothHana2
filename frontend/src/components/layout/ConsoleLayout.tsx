...
export function ConsoleLayout({ role }: { role: 'CREATOR' | 'ADMIN' }) {
  ...
  if (user.role !== role) return <div className="state-panel error-state"><h2>접근 권한이 없습니다</h2><p>현재 계정은 이 관리 화면을 사용할 수 없습니다.</p><NavLink className="btn secondary" to="/">팬 홈으로</NavLink></div>

  return <div className="console-shell">
    <header className="topbar console-topbar"><NavLink className="brand" to="/"><span className="brand-mark">BH</span><span>BoothHana2</span></NavLink><span className="chip muted">{title}</span></header>
    ...
    <aside className="sidebar"><p className="eyebrow">{title}</p><strong className="sidebar-user">{user.displayName}</strong><nav className="side-nav">{links.map(([to, label]) => <NavLink key={to} className="side-link" to={to} end={to === '/creator'}>{label}</NavLink>)}</nav><NavLink className="side-link" to="/">공개 화면</NavLink></aside>
