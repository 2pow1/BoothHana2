...
public class PlatformService {
    ...
    public List<EventView> creatorEvents(UserAccount owner) {
        List<Long> boothIds = booths.findByOwnerUserIdOrderByIdDesc(owner.id).stream().map(value -> value.id).toList();
        ...
        return events.findAllByOrderByStartAtDesc().stream().map(value -> eventView(value, applications.get(value.id))).toList();
    }
    ...
    private EventView eventView(Event value, ApplicationStatus applicationStatus) { long count = eventBooths.findAllByOrderByIdDesc().stream().filter(item -> Objects.equals(item.eventId, value.id) && item.status == ApplicationStatus.APPROVED).count(); return new EventView(value.id, value.name, value.startAt, value.endAt, value.venue, value.description, image(value.imageKey), value.reservationStartAt, value.reservationEndAt, value.status, count, applicationStatus); }
