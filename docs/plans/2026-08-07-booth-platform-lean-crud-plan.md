...
- 역할은 위 세 가지로 고정하며 세부 권한 체계는 만들지 않는다.
- 팬과 크리에이터는 카카오 OAuth를 사용한다. 관리자는 지정된 카카오 계정 ID에만 역할을 부여한다.

## Core user flow

1. 관리자가 행사를 등록하고 공개한다.
2. 크리에이터가 기본 부스를 만들고 행사 참가를 신청하며, 관리자가 승인 또는 반려한다.
3. 승인된 크리에이터가 행사별 부스 정보, 상품, 통합 재고, 예약 가능 여부, `SOLD OUT`, 부스 공지를 관리한다.
4. 팬이 공개 행사에서 부스와 상품을 탐색하고 상품 수량을 예약한 뒤 예약번호와 QR을 확인한다.
5. 팬은 자신의 예약을 조회·취소하고, 크리에이터는 예약번호로 예약을 찾아 수령 완료 처리한다.
6. 크리에이터가 현장 POS 판매를 기록하거나 잘못된 판매를 취소하고 통합 재고를 확인한다.
7. 행사가 종료되면 기존 공개 주소에서 부스·상품 정보를 읽기 전용으로 유지하고 신규 예약과 신규 POS 판매를 막는다. 기존 예약 취소는 허용한다.

## Included scope

- React 프런트엔드, Java 백엔드 API, Supabase PostgreSQL을 분리한 웹 애플리케이션과 실제 저장·재조회
- 카카오 OAuth 기반 팬·크리에이터 로그인과 최소 소유권 구분
...
- 세분화된 권한, 주최자 계정, 관리자 사용자 관리, 감사 로그, 복구 기능
...
| --- | --- | --- |
| `User(id, kakao_subject, role, display_name, created_at)` | 카카오 계정 식별과 팬·크리에이터·관리자 역할 구분 | 최신 안건 정리 46-63행; 엑셀 `03_데이터구조!A1:F16` |
...
- 팬과 크리에이터가 카카오 로그인 후 자신의 역할 화면에 진입하고, 다른 크리에이터의 관리 데이터는 수정할 수 없다.
...
- **I-001 관리자 계정:** 지정된 카카오 계정 ID만 관리자 역할을 받는다. 실제 ID는 구현 단계에서 서버 환경변수 또는 초기 DB 값으로 제공한다.
...
| D-003 | 팬·크리에이터 로그인은 카카오 OAuth만 사용하고 고급 계정 상태를 제외한다. | 최신 안건 정리 46-63행; 개발용 Kakao Developers 앱 준비 확인(2026-08-07) | 이메일 회원가입; 이메일 인증·차단 | 최신 합의에 직접 명시되어 있고 테스트 앱이 준비되었다. | Moderate | user-approved |
...
- Included scope, Explicit exclusions, 기술 구성은 2026-08-09 사용자 승인 완료 상태다.
- 프로젝트는 현재 빈 저장소이며 기존 코드·DB·인증·문서 관례가 없다.
